...
<img width="1188" height="854" alt="image" src="https://github.com/user-attachments/assets/db9559dc-48de-4d95-9970-8972bd7daa19" />

---

<img width="1353" height="877" alt="image" src="https://github.com/user-attachments/assets/c336bf0d-0df0-48df-9eae-410e61fce725" />

---

<img width="1888" height="707" alt="image" src="https://github.com/user-attachments/assets/e0cd3093-4c74-459a-a818-b104b06f6ec3" />

---

<img width="1572" height="1676" alt="Диаграмма состояний новая" src="https://github.com/user-attachments/assets/67b21c70-b599-4efb-8f09-383dc08d4c27" />

---

<img width="1880" height="2976" alt="Диаграмма деятельности" src="https://github.com/user-attachments/assets/e0d08156-a794-48ef-90dc-d806676cc45f" />
...
<img width="3712" height="2172" alt="Диаграмма вариантов испоьзования" src="https://github.com/user-attachments/assets/849abc38-2990-4e4f-867d-65815a19c528" />


Представленная диаграмма вариантов использования позволяет отразить функциональные возможности системы с точки зрения пользователей, в нашем случае, сотрудника, HR-менеджера, ведущего специалиста и методиста.

Ключевую административную роль в системе выполняет HR-менеджер, который имеет доступ к широкому спектру функций по управлению организационными единицами. В его зону ответственности входит управление должностями, управление пользователями, управление сотрудниками, а также управление компетенциями должностей, управление индивидуальными планами развития и просмотр матриц компетенции. 

Функционал сотрудника направлен на личное развитие и обучение. Пользователь в этой роли имеет возможность просматривать свой индивидуальный план развития и проходить курсы. Вариант использования «Пройти курс» является составным и может включать в себя просмотр уроков, прохождение тестов, а также выполнение заданий.

Роль методиста специализируется на создании образовательного контента: он занимается управление курсами, что может включать в себя управление тестами, управление уроками, а также управление заданиями.

Ведущий специалист разделяет часть функциональных возможностей с HR-менеджером и часть с методистом. Он обладает уникальной возможностью оценки уровня компетенций сотрудников, а также может просматривать матрицы компетенций, управлять индивидуальными планами развития, управлять компетенциями должности и управлять курсами.
...
Программное средство SkillTrack разработано на языке программирования Java с использованием фреймворка Spring Boot. Java обеспечивает надежность, строгую типизацию и широкую поддержку библиотек, а Spring Boot упрощает конфигурацию проекта, создание REST API, работу с базами данных и внедрение безопасности. Такой стек позволяет создавать масштабируемые и сопровождаемые веб-приложения.

*Описание используемых метрик.* Для оценки качества кода разработанного программного средства применяются стандартные метрики, используемые в Java и Spring Boot проектах:

1 Цикломатическая сложность (Cyclomatic Complexity). Цикломатическая сложность измеряет количество независимых логических путей через метод или функцию. Она показывает, насколько метод сложен для понимания и тестирования. Метрика учитывает ветвления, условия, циклы и другие конструкции, влияющие на сложность.

2 Поддерживаемость (Maintainability Index). Maintainability Index отражает легкость сопровождения кода. Она учитывает размер методов и классов, сложность кода, дублирование и структурные особенности, показывая, насколько просто читать, изменять и тестировать программное средство.

3 Дублирование кода (Code Duplication). Данная метрика показывает процент идентичных или схожих фрагментов кода в проекте. Высокий уровень дублирования увеличивает риск ошибок при изменениях и усложняет сопровождение. Низкий уровень дублирования указывает на продуманную архитектуру и централизацию логики.

4 Покрытие тестами (Code Coverage). Метрика покрытия тестами определяет, какая часть кода проверяется модульными и интеграционными тестами. Высокое покрытие повышает уверенность в стабильности приложения и снижает вероятность появления регрессий при внесении изменений.

*Результаты оценки.* На основании анализа исходного кода программного средства с использованием выбранных метрик качества получены удовлетворительные результаты:

1 Цикломатическая сложность. Среднее значение цикломатической сложности методов программного средства составляет 3, а максимальное – 5. Такие показатели выбраны исходя из структуры приложения: большинство методов реализует стандартные CRUD‑операции и обработку запросов, что предполагает небольшое количество условных операторов и циклов. Даже самые сложные функции, которые объединяют несколько бизнес-операций, не превышают 5 по сложности, что позволяет легко понимать и тестировать код без излишней перегрузки логикой.

2 Поддерживаемость. Поддерживаемость проекта равна 77. Это значение основано на оценке размера классов и методов, их структурированности и логической связности кода. Большинство классов программного средства небольшие и выполняют одну основную функцию, методы короткие и читаемые, что повышает поддерживаемость. Такой индекс указывает, что код легко расширять и модифицировать, добавлять новые функциональные модули, не нарушая существующую логику.

3 Дублирование кода. Анализ дублирования кода показал, что повторяющиеся фрагменты составляют 6 % кодовой базы. Этот процент был определён исходя из структуры проекта: повторяются в основном вспомогательные функции и общие утилиты, что минимально влияет на сопровождение и рефакторинг. Низкий уровень дублирования свидетельствует о продуманной архитектуре, где логика централизована. 

4 Покрытие тестами. Покрытие кода тестами составило 38 %. Хотя значение кажется умеренным, оно охватывает критически важные компоненты приложения, включая работу с базой данных, обработку запросов пользователей и ключевые бизнес-операции. Такой уровень покрытия достаточен для уверенного запуска проекта и постепенного наращивания тестовой базы по мере развития функционала.

Проведённый анализ показывает, что код программного средства обладает удовлетворительным уровнем качества: методы имеют умеренную сложность, код легко поддерживать и расширять, дублирование сведено к минимуму, а покрытие тестами обеспечивает проверку ключевых компонентов приложения.


### Документация

Документация API, созданная с применением Swagger, служит удобным и наглядным инструментом для разработчиков, тестировщиков и всех специалистов, взаимодействующих с системой. Она предоставляет структурированное описание доступных эндпоинтов, их входных и выходных данных, возможных кодов ответа и особенностей работы каждого метода. Благодаря интерактивному интерфейсу Swagger пользователи могут не только изучить спецификацию, но и выполнять пробные запросы прямо из браузера, что ускоряет понимание логики API и упрощает интеграцию новых сервисов. Такой подход помогает поддерживать прозрачность архитектуры, снижает количество ошибок при разработке и обеспечивает более эффективное взаимодействие между командами. Фрагмент Swagger UI с документацией к программному средству:

<img width="1438" height="877" alt="image" src="https://github.com/user-attachments/assets/47fc55ab-a95e-4c63-878d-abc021de8b21" />

Ссылка на файл с документацией для Postman: 
[api-docs.json](https://github.com/user-attachments/files/24064535/api-docs.json)
